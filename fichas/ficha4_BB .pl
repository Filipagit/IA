...
%--------------------------------- 
%alinea 2) -- ?- g(G),caminho(G,braga,lamego,P).

caminho(G,A,B,P) :- caminho1(G,A,[B],P).

caminho1(_,A,[A|P1],[A|P1]).
caminho1(G,A,[Y|P1],P) :- adjacente(X,Y,_,_,G) , nao(membro(X,[Y|P1])) , caminho1(G,A,[X,Y|P1],P).

%--------------------------------- 
% alinea 3) -- ?- g(G),ciclo(G,viseu,P).
...
%--------------------------------- 
%alinea 4) ?- g(G),caminhoK(G,braga,guarda,P,K,E).
...
%--------------------------------- 
%alinea 5) ?- g(G),cicloK(G,viseu,P,K,E).

cicloK(G,A,P,Km,[E|Ep1]) :- adjacente(B,A,E,K,G), caminhoK(G,A,B,P1,Kp1,Ep1) , length(P1,L) , L>2,
    append(P1,[A],P), Km is K + Kp1. 
